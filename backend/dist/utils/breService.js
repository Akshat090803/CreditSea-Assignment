export const runBusinessRuleEngine = (data) => {
    const age = calculateAge(new Date(data.dob));
    if (age < 23 || age > 50)
        return { passed: false, error: "Age must be between 23 and 50." };
    if (data.monthlySalary < 25000)
        return { passed: false, error: "Monthly salary must be at least 25,000." };
    if (data.employmentMode === 'Unemployed')
        return { passed: false, error: "Applicant cannot be unemployed." };
    const panRegex = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;
    if (!panRegex.test(data.pan))
        return { passed: false, error: "Invalid PAN format." };
    return { passed: true };
};
...
export const calculateRepayment = (principal, tenureDays) => {
